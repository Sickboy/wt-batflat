...
//	$this -> _showGallery();
	...
	if (isset($_GET['g'])){
		...
		$galleryName = $this->db('galleries')->select('name')->select('desc')->where('slug', $_GET['g'])->limit(1)->oneArray();
		...
		if (count($items)) {
                   ...
                   $assign = $this->draw('gallery.html', ['items' => $items, 'galleryList' => $galleryList, 'name' => $galleryName ]);
...
//		print_r($galleryFirstTmp);
		...
		$galleryName = $this->db('galleries')->select('name')->select('desc')->where('id', $galleryFirst)->oneArray();
