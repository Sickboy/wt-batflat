	...
	foreach($rows as $row)
	{
		$data[] = array(
		 ...
		 'color' => $row["color"]
	        ...
	        $rows = $this->db('moonphase')->toArray();
        foreach($rows as $row)
        {
                $data[] = array(
                 'id'   => $row["id"],
...
//              'end'   => "",
    ...
    {
        return $this->draw('moonphase.html');
    }
    public function anySafe()
    {
	$query = $this->db('calendar')->save(['title' => $_POST['tytul'], 'desc' => $_POST['tresc'] ,'start' => $_POST['od'], 'stop' => $_POST['do'],'user_id' => $this->core->getUserInfo('id') ,'color' => $_POST['kolor']]);
