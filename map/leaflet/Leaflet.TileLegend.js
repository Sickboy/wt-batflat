L.TileLegend = L.Class.extend({
    ...
    initialize: function (map, tilelayer, container) {
        ...
        maximize.title = "Powiększ legende";
        minimize.title = "Pomniejsz legende";
        close.title = "Zamknij legende";
    ...
    buildKey: function (key, container, section) {
        ...
        L.DomEvent.on(container, 'click', function (e) {
			...
			// --- W zależności od nazwy podsekcji otwiera popup markera o nazwie mar_xxx[id]
			
			if (section.title == 'Ambony'){
			mar_amb[id].openPopup();};
			
			if (section.title == 'Moje punkty na mapie'){
			mar_point[id].openPopup();};
			
			if (section.title == 'Zwyżki'){
			mar_zw[id].openPopup();};
			
			if (section.title == 'Paśniki'){
			mar_pas[id].openPopup();};
			
			if (section.title == 'Ksiażki pobytu myśliwych na polowaniu'){
			mar_lp[id].openPopup();};
